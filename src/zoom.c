...
/*   Updated: 2017/11/25 04:53:06 by thvocans         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_jul_zoom(t_mlx *w, long double x, long double y , float zoom)
{
	...
	j = &w->jul;
	j->zoom *= zoom;
	j->moveX += (x - LARG / 2) / LARG / j->zoom ;
//	j->moveX += (x / LARG * j->zoom * 1.5 ;
	j->moveY += (y - (HAUT / 2)) / HAUT / j->zoom;
	printf("%Lf, %Lf\n",1.5 * (x - LARG / 2) / (j->zoom * LARG / 2) + j->moveX,
			(y - HAUT / 2) / (0.5 * j->zoom * HAUT) + j->moveY);
	clear_img(&w->pic2);
	ft_julia(w);
}
